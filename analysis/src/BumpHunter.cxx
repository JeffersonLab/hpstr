...
/** 
 * @file BumpHunter_global.cxx
 * @brief
 * @author Omar Moreno, Emrys Peets
 *
 */
...
HpsFitResult* BumpHunter::performSearch(TH1* histogram, double mass_hypothesis, bool skip_bkg_fit, bool skip_ul) {
    ...
    //setting poly order here as the global fit function is not a polynomial 
    ...
    if(!skip_bkg_fit) {
        ...
        if(isChebyshev or isLegendre) {
            ...
            TFitResultPtr result = histogram->Fit("bkg_global", "QLES+", "", window_start_, window_end_);
            fit_result->setBkgFitResult(result);
    ...
    void BumpHunter::getUpperLimitAsymCLs(TH1* histogram, HpsFitResult* result) {
        ...
        //  Get the signal yield obtained from the signal+bkg fit
            ...
            double mu_hat = result->getCompFitResult()->Parameter(poly_order_ );
            double sigma = result->getCompFitResult()->ParError(poly_order_ );
        }
        ...
        while(true) {
            ...
            if(isChebyshev || isLegendre){
                ...
                comp->FixParameter(poly_order_ , mu95up);
            }
