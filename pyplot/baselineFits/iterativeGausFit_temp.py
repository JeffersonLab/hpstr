...
def getGausFitParameters(inFile):
    ...
    return channel,mean, sigma, histo_key, iterativeFit_mean, iterativeFit_chi2_2ndDerivative, iterativeFit_2Der_range, iterativeFit_chi2_NDF, iterativeFit_range_end
...
def buildTGraph(name,title, n_points, x, y):
...
def newCanvas(name):
        ...
        return canvas
def savePNG(canvas,plot,directory,name):
        canvas.cd()
        plot.Draw()
        canvas.Draw()
...
for k in histokeys_hh:
    ...
    print k
    channel, mean, sigma, histo_key, iterativeFit_mean, iterativeFit_chi2_2ndDerivative, iterativeFit_2Der_range, iterativeFit_chi2_NDF, iterativeFit_range_end = getGausFitParameters(inFile)
    n = len(channel)
    m =0
    x=[]
    y=[]
        ...
        for e in channels_in:
            for i in range(len(channel)):
                if k == histo_key(i)+"_hh" and i==e:
                    print iterativeFit_mean[i]
                    #chi2NDF_g = buildTGraph("Chi2_%s_channel_%i"%(k,i),"Chi2_vs_FitRangeEnd_%s_channel_%i;FitRangeEnd;chi2NDF"%(k,i),len(
