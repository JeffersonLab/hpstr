...
for fitData in myTree:
    ...
    if fitData.minbinFail == 0.0:
        ...
        lowdaqposDict.setdefault(sensor,[]).append(fitData.channel)
    ...
    if fitData.minbinFail == 1.0:
        ...
        nofitposDict.setdefault(sensor,[]).append(fitData.channel)
...
print nofitposDict
csvfile = "test.csv"
zd = zip(*nofitposDict.values())
try:
    with open(csvfile,'wb') as outfile:
        writer = csv.writer(outfile, delimiter=',')
        writer.writerow(nofitposDict.keys())
        writer.writerows(zd)
       # for data in nofitposDict:
