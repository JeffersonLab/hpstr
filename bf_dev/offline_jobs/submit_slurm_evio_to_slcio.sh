...
mkdir $RUNDIR/../logs
cd $RUNDIR
/usr/bin/python3.6 ${HPSMC_DIR}/lib/python/hpsmc/job.py run -o $RUNDIR/../logs/job.${JOB_ID}.out -e $RUNDIR/../logs/job.${JOB_ID}.err -l $RUNDIR/../logs/job.${JOB_ID}.log -d $RUNDIR -c $JOBDIR/.hpsmc -i $JOB_ID ${HPSMC_DIR}/../python/jobs/data_cnv_job.py $JOBDIR/jobs_${run}.json
