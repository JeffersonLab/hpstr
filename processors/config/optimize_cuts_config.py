...
def radiativeFraction(mass):
    radF = -1.04206e-01 + 9.92547e-03*mass + -1.99437e-04*pow(mass, 2) + 1.83534e-06*pow(
        # alic 2016 simps kf 11/15/22
        mass, 3) + -7.93138e-9*pow(mass, 4) + 1.30456e-11*pow(mass, 5)
    return radF
...
def radiativeAcceptance(mass):
    acc = (-7.35934e-01 + 9.75402e-02*mass + -5.22599e-03*pow(mass, 2) + 1.47226e-04*pow(mass, 3) + -2.41435e-06*pow(mass, 4) + 2.45015e-08*pow(mass,
           # alic 2016 simps kf 11/15/22
                                                                                                                                                5) + -1.56938e-10*pow(mass, 6) + 6.19494e-13*pow(mass, 7) + -1.37780e-15*pow(mass, 8) + 1.32155e-18*pow(mass, 9))
    return acc
...
zbi.parameters['variableHistCfgFilename'] = hpstr_analysis_base + \
    'plotconfigs/optimize/zbi_optimization_histograms.json'
# Config SIMP model
