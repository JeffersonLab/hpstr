...
void TrackHitCompareAnaProcessor::initialize(TTree* tree) {
    ...
    if (isMC_ == 1){
        ...
        chargesOnTrk1_ = new TH1F("chargesOnTrk", "MC Charge Distribution for On Track", 1000, 0.0, 0.000016);
    ...
    chargesOnTrk1_ = new TH1F("chargesOnTrk", "Charge Distribution for On Track", 1000, 0.0, 0.000016);
    ...
    chargesOnTrkNTD1_ = new TH1F("chargesOnTrkNTD", "Charge Distribution for On Track", 1000, 0.0, 0.000016);
    ...
    // Instantiating the second layer
    ...
    chargesOnTrk2_ = new TH1F("chargesOnTrk", "Charge Distribution for On Track", 1000, 0.0, 0.000016);
    ...
    chargesOnTrkNTD2_ = new TH1F("chargesOnTrkNTD", "Charge Distribution for On Track", 1000, 0.0, 0.000016);
...
bool TrackHitCompareAnaProcessor::process(IEvent* ievent) {
    if (doingTracks_){
        for (int i = 0; i<tracks_->size(); i++){
            ...
            if (track->getTrackTime()*track->getTrackTime()<100.0){
                if (ident_ < 1.5){
                    Z0VNShare2Hist1_->Fill(track->getZ0Err(), track->getNShared());
                } else {
                    Z0VNShare2Hist2_->Fill(track->getZ0Err(), track->getNShared());
                }
                if (track->getP()<pcut_){
                    if (ident_ < 1.5){
                        Z0VNShare2HistCut1_->Fill(track->getZ0Err(), track->getNShared());
                    } else {
                        Z0VNShare2HistCut2_->Fill(track->getZ0Err(), track->getNShared());
                    }
                }
            }
            ...
            if (ident_ < 1.5) {
                TrackMomentumAllTime1_->Fill(track->getP());
            } else {
                TrackMomentumAllTime2_->Fill(track->getP());
            }
        
            if (track->getTrackTime()*track->getTrackTime() < 16.0){
                if (ident_ < 1.5){
                    TrackMomentumInTime1_->Fill(track->getP());
                } else {
                    TrackMomentumInTime2_->Fill(track->getP());
                }
            } else {
                if (ident_ < 1.5) {
                    TrackMomentumOutTime1_->Fill(track->getP());
                } else {
                    TrackMomentumOutTime2_->Fill(track->getP());
                }
            }
          
            // Transverse Track Momentum for same cuts
            if (ident_ < 1.5){
                TrackMomentumTAllTime1_->Fill(track->getPt());
            } else { 
                TrackMomentumTAllTime2_->Fill(track->getPt());
            }
        
            if (track->getTrackTime()*track->getTrackTime() < 16.0){
                if (ident_ < 1.5){
                    TrackMomentumTInTime1_->Fill(track->getPt());
                } else {
                    TrackMomentumTInTime2_->Fill(track->getPt());
                }
            } else {
                if (ident_ < 1.5){
                    TrackMomentumTOutTime1_->Fill(track->getPt());
                } else {
                    TrackMomentumTOutTime2_->Fill(track->getPt());
                }
            }
        }
    }
    for(int i = 0; i < Clusters_->size(); i++){ 
        ...
        if (doingTracks_){
            ...
            if (increment > 0){
                ...
                if (((layc==layer_)&&(modc==module_))||(general)){
                    if (isShared){
                        if (ident_ < 1.5){
                            SharedAmplitudes1_->Fill(clu->getCharge());
                        } else {
                            SharedAmplitudes2_->Fill(clu->getCharge());
                        }
                        if (ident_ < 1.5){
                            SharedTimes1_->Fill(clu->getTime());
                        } else {
                            SharedTimes2_->Fill(clu->getTime());
                        }
                    } else {
                        if (ident_ < 1.5){
                            UnSharedAmplitudes1_->Fill(clu->getCharge());
                        } else {
                            UnSharedAmplitudes2_->Fill(clu->getCharge());
                        }
                        if (ident_ < 1.5){
                            UnSharedTimes1_->Fill(clu->getTime());
                        } else {
                            UnSharedTimes2_->Fill(clu->getTime());
                        }
                    }
                }
            }
        ...
        if (((layc==layer_)&&(modc==module_))||(general)){
            ...
            if (dist < 69420){
                if (ident_ < 1.5){
                    ClusDistances1_->Fill(dist);
                } else {
                    ClusDistances2_->Fill(dist);
                }
                if (NTD){
                    if (ident_ < 1.5){
                        ClusDistancesNTD1_->Fill(dist);
                    } else {
                        ClusDistancesNTD2_->Fill(dist);
                    }
                }
            }
            if (ident_ < 1.5){
                layers1_->Fill(nLayers);
                charges1_->Fill(ncharges);
                positions1_->Fill(clu->getLayer());
                times1_->Fill(ntimes);
            } else {
                layers2_->Fill(nLayers);
                charges2_->Fill(ncharges);
                positions2_->Fill(clu->getLayer());
                times2_->Fill(ntimes);
            }
            if (onTrk){
                if (ident_ < 1.5){
                    layersOnTrk1_->Fill(nLayers);
                    chargesOnTrk1_->Fill(ncharges);
                    timesOnTrk1_->Fill(ntimes);
                    positionsOnTrk1_->Fill(clu->getLayer());
                } else {
                    layersOnTrk2_->Fill(nLayers);
                    chargesOnTrk2_->Fill(ncharges);
                    timesOnTrk2_->Fill(ntimes);
                    positionsOnTrk2_->Fill(clu->getLayer());
                }
            } else {
                if (ident_ < 1.5){
                    layersOffTrk1_->Fill(nLayers);
                    chargesOffTrk1_->Fill(ncharges);
                    timesOffTrk1_->Fill(ntimes);
                } else {
                    layersOffTrk2_->Fill(nLayers);
                    chargesOffTrk2_->Fill(ncharges);
                    timesOffTrk2_->Fill(ntimes);
                }
            }
            if (NTD){
                if (ident_ < 1.5){
                    layersNTD1_->Fill(nLayers);
                    chargesNTD1_->Fill(ncharges);
                    timesNTD1_->Fill(ntimes);
                } else {
                    layersNTD2_->Fill(nLayers);
                    chargesNTD2_->Fill(ncharges);
                    timesNTD2_->Fill(ntimes);
                }
                if (onTrk){
                    if (ident_ < 1.5){
                        layersOnTrkNTD1_->Fill(nLayers);
                        chargesOnTrkNTD1_->Fill(ncharges);
                        timesOnTrkNTD1_->Fill(ntimes);
                    } else {
                        layersOnTrkNTD2_->Fill(nLayers);
                        chargesOnTrkNTD2_->Fill(ncharges);
                        timesOnTrkNTD2_->Fill(ntimes);
                    }
                } else {
                    if (ident_ < 1.5){
                        layersOffTrkNTD1_->Fill(nLayers);
                        chargesOffTrkNTD1_->Fill(ncharges);
                        timesOffTrkNTD1_->Fill(ntimes);
                    } else {
                        layersOffTrkNTD2_->Fill(nLayers);
                        chargesOffTrkNTD2_->Fill(ncharges);
                        timesOffTrkNTD2_->Fill(ntimes);
                    }
                } 
            }
        }      
