...
void FlatVertexProcessor::initialize(TTree* tree) {
    ...
    for (unsigned int i_reg = 0; i_reg < regionSelections_.size(); i_reg++) {
        ...
        for( int l=1; l<15; l++ ){
            _reg_tuples[regname]->addVariable(TString::Format("unc_vtx_ele_track_layer%i",l).Data() );
            _reg_tuples[regname]->addVariable(TString::Format("unc_vtx_pos_track_layer%i",l).Data() );
        }
...
bool FlatVertexProcessor::process(IEvent* ievent) {
    ...
    vertsPassNominal+=selected_vtxs.size(); 
    ...
    for (auto region : _regions) {
        ...
        for (std::vector<Vertex*>::iterator it = goodVtxs.begin(); it != goodVtxs.end(); it++) {
	      ...
	      std::cout<<" for some reason we get empty vertex or particles from vtx?????"<<std::endl;
	      continue;
	    }
            CalCluster eleClus = ele->getCluster();
            CalCluster posClus = pos->getCluster();
	    ...
	    //            for( int l=1; l<15; l++){
            for( int l=0; l<15; l++){
                ...
                _reg_tuples[region]->setVariableValue(TString::Format("unc_vtx_ele_track_layer%i",l).Data(),hasHit);
            }
	    int n_pos_hits = 0;
	    //            for( int l=1; l<15; l++){
            for( int l=0; l<15; l++){
                ...
                _reg_tuples[region]->setVariableValue(TString::Format("unc_vtx_pos_track_layer%i",l).Data(),hasHit);
            }
	    //just get it from the object
	    double nEleHits=ele_trk.getTrackerHitCount();
	    double nPosHits=pos_trk.getTrackerHitCount();
	    
	    //	    std::cout<<" Number of electron hits = "<<nEleHits<<std::endl;
	    //std::cout<<" Number of positron hits = "<<nPosHits<<std::endl;
            ...
            _reg_tuples[region]->setVariableValue("unc_vtx_ele_track_chi2", ele_trk.getChi2());
	    ...
	    //            _reg_tuples[region]->setVariableValue("unc_vtx_ele_track_nhits",n_ele_hits); //ele2dHits);
            _reg_tuples[region]->setVariableValue("unc_vtx_ele_track_nhits",nEleHits); //ele2dHits);
            ...
            _reg_tuples[region]->setVariableValue("unc_vtx_pos_track_chi2", pos_trk.getChi2());
	    ...
	    //            _reg_tuples[region]->setVariableValue("unc_vtx_pos_track_nhits",n_pos_hits); //pos2dHits);
            _reg_tuples[region]->setVariableValue("unc_vtx_pos_track_nhits",nPosHits); //pos2dHits);
...
void FlatVertexProcessor::finalize() {
  std::cout<<"  Total Number of Vertices = "<<totalVerts<<std::endl;
  std::cout<<"  Vertices passed selection= "<<vertsPassNominal<<std::endl;
  outF_->cd();
