...
//I will comment out things I do not think need to be in the file, and will edit other statements accordingly
//
...
//I should note that it could be HPSEvioReader(string infile="",string trigfile="",int dataset=2019) with three parameters instead of 2…
HPSEvioReader::HPSEvioReader(int argc, const char * argv[]) {
  Arguments_t args;
  args.Parse_Args(&argc,argv);
  
  //HPSEvioReader *etool=new HPSEvioReader(args.filenames[0].c_str());
  //etool -> Open does BLANKNKNKNK  
  //we could have also had  “etool->SVT->fStoreRaw=true;” to make the processing faster. But it is nice to read the SVT headers into storage.
