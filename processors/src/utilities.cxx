...
Particle* utils::buildParticle(EVENT::ReconstructedParticle* lc_particle, std::string trackstate_location,
                               EVENT::LCCollection* gbl_kink_data, EVENT::LCCollection* track_data)

{
    if (!lc_particle) return nullptr;
...
bool utils::IsSameTrack(Track* trk1, Track* trk2) {
    double tol = 1e-6;
    if (fabs(trk1->getD0() - trk2->getD0()) > tol || fabs(trk1->getPhi() - trk2->getPhi()) > tol ||
        fabs(trk1->getOmega() - trk2->getOmega()) > tol || fabs(trk1->getTanLambda() - trk2->getTanLambda()) > tol ||
        fabs(trk1->getZ0() - trk2->getZ0()) > tol || fabs(trk1->getChi2Ndf() - trk2->getChi2Ndf()) > tol)
        return false;

    return true;
}

Track* utils::buildTrack(EVENT::Track* lc_track, std::string trackstate_location, EVENT::LCCollection* gbl_kink_data,
                         EVENT::LCCollection* track_data) {
    if (!lc_track) return nullptr;
    ...
    std::map<std::string, int> trackstateLocationMapV30_ = {{"", EVENT::TrackState::AtPerigee},
    ...
    std::map<std::string, int> trackstateLocationMapV23_ = {
        {"", EVENT::TrackState::AtIP}, {"AtIP", 1},     {"AtFirstHit", 2},  {"AtLastHit", 3},
        {"AtCalorimeter", 4},          {"AtVertex", 5}, {"LastLocation", 5}};
    ...
    if (it != trackstateLocationMap_.end()) {
        ...
        std::cout << "[utilities]::ERROR Track State Location " << trackstate_location << " Doesn't Exist!"
                  << std::endl;
    ...
    track->setTrackParameters(tsD0, ts->getPhi(), ts->getOmega(), ts->getTanLambda(), tsZ0);

    double position[3] = {ts->getReferencePoint()[1], ts->getReferencePoint()[2], ts->getReferencePoint()[0]};
    ...
    if (abs(bLocal) <
        10.0)  //  check if it has non-default value (which should be 666)...this fails for pre-v3 lcio.  see below
    ...
    const EVENT::TrackState* track_state = lc_track->getTrackState(trackstateLocationMap_["AtCalorimeter"]);

    if (track_state) {
        double position_at_ecal[3] = {track_state->getReferencePoint()[1], track_state->getReferencePoint()[2],
    ...
    if (gbl_kink_data) {
        ...
        std::shared_ptr<UTIL::LCRelationNavigator> gbl_kink_data_nav =
            std::make_shared<UTIL::LCRelationNavigator>(gbl_kink_data);

        // Get the list of GBLKinkData associated with the LCIO Track
        EVENT::LCObjectVec gbl_kink_data_list = gbl_kink_data_nav->getRelatedFromObjects(lc_track);
    ...
    if (track_data) {
        ...
        std::shared_ptr<UTIL::LCRelationNavigator> track_data_nav =
            std::make_shared<UTIL::LCRelationNavigator>(track_data);
        ...
        if (track_data_list.size() == 1) {
            ...
            if (track_datum->getNDouble() > 14 || track_datum->getNFloat() > 8 || track_datum->getNInt() != 1) {
                throw std::runtime_error("[ TrackingProcessor ]: The collection " +
                                         std::string(Collections::TRACK_DATA) + " has the wrong structure.");
            }
            ...
            if (track_datum->getNFloat() > 3 &&
                abs(bLocal) > 10.0)  // bfield is not set in track state so get from track data...
                track->setMomentum(track_datum->getFloatVal(1), track_datum->getFloatVal(2),
                                   track_datum->getFloatVal(3));
            ...
            if (abs(bLocal) > 10.0) {  // bfield is not set in track state so get from track data...pre-v3 lcio
                ...
                if (track_datum->getNFloat() > 4) {
                    if (loc == trackstateLocationMap_[""]) bfieldY = track_datum->getFloatVal(4);
                    if (loc == trackstateLocationMap_["AtTarget"]) {
                        bfieldY = track_datum->getFloatVal(5);
                    }
                    if (loc == trackstateLocationMap_["AtCalorimeter"]) bfieldY = track_datum->getFloatVal(6);
...
RawSvtHit* utils::buildRawHit(EVENT::TrackerRawData* rawTracker_hit, EVENT::LCCollection* raw_svt_hit_fits) {
    EVENT::long64 value =
        EVENT::long64(rawTracker_hit->getCellID0() & 0xffffffff) | (EVENT::long64(rawTracker_hit->getCellID1()) << 32);
    ...
    int hit_adcs[6] = {(int)rawTracker_hit->getADCValues().at(0), (int)rawTracker_hit->getADCValues().at(1),
                       (int)rawTracker_hit->getADCValues().at(2), (int)rawTracker_hit->getADCValues().at(3),
                       (int)rawTracker_hit->getADCValues().at(4), (int)rawTracker_hit->getADCValues().at(5)};

    rawHit->setADCs(hit_adcs);
    if (raw_svt_hit_fits) {
        std::shared_ptr<UTIL::LCRelationNavigator> rawTracker_hit_fits_nav =
            std::make_shared<UTIL::LCRelationNavigator>(raw_svt_hit_fits);

        // Get the list of fit params associated with the raw tracker hit
        EVENT::LCObjectVec rawTracker_hit_fits_list = rawTracker_hit_fits_nav->getRelatedToObjects(rawTracker_hit);
        ...
        double fit_params[5] = {(double)hit_fit_param->getDoubleVal(0), (double)hit_fit_param->getDoubleVal(1),
                                (double)hit_fit_param->getDoubleVal(2), (double)hit_fit_param->getDoubleVal(3),
...
TrackerHit* utils::buildTrackerHit(IMPL::TrackerHitImpl* lc_tracker_hit, bool rotate, int type) {
    if (!lc_tracker_hit) return nullptr;
...
bool utils::addRawInfoTo3dHit(TrackerHit* tracker_hit, IMPL::TrackerHitImpl* lc_tracker_hit,
                              EVENT::LCCollection* raw_svt_fits, std::vector<RawSvtHit*>* rawHits, int type,
                              bool storeRawHit) {
    if (!tracker_hit || !lc_tracker_hit) return false;
    ...
    for (unsigned int irh = 0; irh < lc_rawHits.size(); ++irh) {
        ...
        EVENT::TrackerRawData* rawTracker_hit = static_cast<EVENT::TrackerRawData*>(lc_rawHits.at(irh));
        ...
        if (type == 1) currentHitLayer = rawHit->getLayer() - 1;
        if (volume == -1)
            volume = currentHitVolume;
        else {
            if (currentHitVolume != volume)
                std::cout << "[ ERROR ] : utils::addRawInfoTo3dHit raw hits with inconsistent volume found"
                          << std::endl;
        }
...
bool utils::isUsedByTrack(IMPL::TrackerHitImpl* lc_tracker_hit, EVENT::Track* lc_track) {
    ...
    for (auto trk_lc_tracker_hit : trk_lc_tracker_hits) {
        ...
        if (lc_tracker_hit->id() == trk_lc_tracker_hit->id()) return true;
    }
    return false;
}

bool utils::isUsedByTrack(TrackerHit* tracker_hit, EVENT::Track* lc_track) {
    EVENT::TrackerHitVec trk_lc_tracker_hits = lc_track->getTrackerHits();

    for (auto trk_lc_tracker_hit : trk_lc_tracker_hits) {
        if (tracker_hit->getID() == trk_lc_tracker_hit->id()) return true;
    }
    return false;
}
...
double utils::getKalmanTrackL1Isolations(Track* track, std::vector<TrackerHit*>* siClusters) {
    ...
    for (int i = 0; i < track->getSvtHits().GetEntries(); i++) {
        ...
        if (trackhit_volume == 1) {
            if (trackhit_layer % 2 == 1) isAxial = true;
        }
        if (trackhit_volume == 0) {
            if (trackhit_layer % 2 == 0) isAxial = true;
        }
        ...
        if (siClusters == nullptr) return isohit_dy;
        for (int j = 0; j < siClusters->size(); j++) {
            ...
            if (althit_layer != trackhit_layer) continue;

            // Skip if volume doesn't match
            if (althit_volume != trackhit_volume) continue;

            // Skip same hit
            if (althit_id == trackhit_id) continue;

            // Only look at hits that are further from beam-axis in Global Y
            if ((trackhit_volume == 0 && althit_y < trackhit_y) || (trackhit_volume == 1 && althit_y > trackhit_y))
                continue;

            // Require alternative hits to be within +-30ns (based on SiClustersOnTrack t distr)
            if (std::abs(althit_time) > 30.0) continue;
            ...
            if (trackhit_minstrip - althit_maxstrip <= 1 && althit_minstrip - trackhit_maxstrip <= 1) continue;
...
void utils::get2016KFMCTruthHitCodes(Track* ele_trk, Track* pos_trk, int& L1L2hitCode, int& L1hitCode, int& L2hitCode) {
    ...
    if (ele_trk->isKalmanTrack()) {
        for (int i = 0; i < ele_trk->getMcpHits().size(); i++) {
            ...
            if (mcpid == maxID_ele) isGood = true;

            if (isGood) {
                if (isAxial) {
                    if (isL1) ele_trueAxialL1 = true;
                    if (isL2) ele_trueAxialL2 = true;
                }
                if (isStereo) {
                    if (isL1) ele_trueStereoL1 = true;
                    if (isL2) ele_trueStereoL2 = true;
                }
            }
        }
    }
    ...
    if (pos_trk->isKalmanTrack()) {
        for (int i = 0; i < pos_trk->getMcpHits().size(); i++) {
            ...
            if (mcpid == maxID_pos) isGood = true;

            if (isGood) {
                if (isAxial) {
                    if (isL1) pos_trueAxialL1 = true;
                    if (isL2) pos_trueAxialL2 = true;
                }
                if (isStereo) {
                    if (isL1) pos_trueStereoL1 = true;
                    if (isL2) pos_trueStereoL2 = true;
                }
            }
        }
    }
