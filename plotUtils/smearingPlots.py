...
class plotFitter:
    def __init__(self, dataFile, mcFile, outFile, regions):
        ...
        self.sigmaRange = 1.5
        self.rangeFit = [2.2,5.2]
        self.rebinX   = 1
        self.rebinY   = 1
        self.hmap     = {}
        self.testFactor = 1.0   # Only for testing purposes when MC is missing
    ...
    def fit2D(self,hname):
        ...
        self.hmap[hname+"_sigma_mc"]   = h_sigma_mc
        self.hmap[hname+"_sigma_data"] = h_sigma_data

        # Now compute the smearing factor for each bin
        for ibin in range(h_sigma_data.GetXaxis().GetNbins()+1):
            sigma_data = h_sigma_data.GetBinContent(ibin)*self.testFactor
            sigma_mc   = h_sigma_mc.GetBinContent(ibin)

            st,sigmast = self.smearing_term(sigma_data,sigma_mc)

            print(ibin,sigma_data,sigma_mc,st,sigmast)
...
def main():
    ...
    parser.add_option("-m","--mcfile",type="string",dest="mcfile",
