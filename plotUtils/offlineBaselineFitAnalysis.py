...
parser.add_argument("-svtThresholdsFile", type=str, dest="svtThresholdsFile", help="Input thresholds for online run closest but less than current run")
...
def readOfflineFitTuple(inFile, key):
...
#Read SVT DAQ Thresholds file corresponding to this run
svtThresholdsFile = options.svtThresholdsFile
...
onlineBaselinesLoaded = False
if(options.onlineBaselines) != "":
    onlineBaselinesLoaded = True
...
for entry in hybridHwDict:
    ...
    offlineBaselinesTuple = readOfflineFitTuple(inFile, hh.GetName())
    ...
    #If you want to compare online to offline baselines, load online baseline
    #Get Online Baseline Fit Tuple
    onlineBaselinesTuple = ()
    if onlineBaselinesLoaded:
    ...
    #For future run with high lumi, we may want to generate SVT Daq thresholds using offline data
    '''
    if onlineBaselinesTuple:
        if(int(febn) == 5 and int(hybn) == 1):
            F5H1Thresholds(svtThresholdsFile, threshOutFile)
        else:
            generateThresholds(threshOutFile, outputFile, offlineBaselinesTuple, onlineBaselinesTuple, febn, hybn, hybrid)
    '''
