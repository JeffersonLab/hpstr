...
SubDirs=[]
SubDirNames=[]
for i in range(0,len(DFS.GetListOfKeys())):
    print(type(DFS.Get(DFS.GetListOfKeys().At(i).GetName())))    
    if(len(str(type(DFS.Get(DFS.GetListOfKeys().At(i).GetName()))))==45):   
        SubDirs.append(DFS.Get(DFS.GetListOfKeys().At(i).GetName()))
        SubDirNames.append(DFS.GetListOfKeys().At(i).GetName())
...
#FIRST THE MODULE HTMLS
#THEN THE REGIONS SubDirs/SubDirNames
#THEN THE TYPE OF HIST 
#THEN PHASES

Layers=[]
for j in range(len(SubDirs[i].GetListOfKeys())):
    word=SubDirs[0].GetListOfKeys().At(j).GetName()
    word=word[word.find('L'):]
    Layers.append(word)
print(Layers)
