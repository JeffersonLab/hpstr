...
index = 0
for f in sorted(glob.glob('/sdf/group/hps/users/alspellm/projects/simps_2019/mc/simps/gen/recon/tuple_ana/hadd_mass*ana.root')):
    ...
    psum_h.SetName('%s_mass_VD_Psum' % (mass))
    psum_h.SetTitle('%s_mass_VD_Psum;vtx P_{sum} [GeV];NVertices' % (mass))
    psum_h.Rebin(50)
    psum_h.SetLineWidth(2)
    psum_h.SetLineColor(colorsMap[index])
    psum_h.GetYaxis().SetRangeUser(0.001, 2000)
...
outfile = r.TFile("out_psum_mass.root", "RECREATE")
outfile.cd()
c.cd()
